...
export function StoreContextProvider({ children }) {
  ...
  const addToCart = (item, cant, data) => {
    ...
    const newCart = cart.map((cartItem) => {
      if (cartItem.id === item.id) {
        if (
          cartItem.selectedSizeId === data.selectedSizeId &&
          cartItem.selectedColorId === data.selectedColorId
        ) {
          let copyItem = { ...cartItem };
          copyItem.quantity += cant;
          console.log(
            "producto existente en el carrito con mismo size y color, agregando mas quantity",
            copyItem
          );
          itemExists = true;
          return copyItem;
  ...
  const handleBuy = async () => {
    ...
    console.log(
      "ID y VALUE del option seleccionado: " + selectedOptionId,
      selectedValue
    );
