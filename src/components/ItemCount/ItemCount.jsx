...
function ItemCount({ onAdd, producto }) {
      ...
      if (findSize) {
        ...
        if (findColor) {
          const productInCart = cart.find(
            (c) => c.selectedColorId == findColor.id
          );
          if (productInCart) {
            console.log(
              "nuevo stock dependiendo del item en carrito",
              findColor.stock
            );
  ...
  const onSubmit = (data) => {
    const id = toastLoading("Añadiendo producto al carrito");
    const findItem = cart.find(
      (c) => c.selectedColorId == data.selectedColorId
    );
    if (!findItem) {
      console.log(
        "antes de agregar al carrito producto no encontrado en el carrito",
        data
      );
      ...
      setTimeout(() => {
        return toastSuccess(
          <>
            Producto añadido: <strong>'{producto.name}'</strong>
          </>,
          id
        );
      }, 1000);
      ...
      if (count <= availableStock) {
        console.log(
          "antes de agregar al carrito se encontro el prod en el carrito",
          data
        );
        const itemAgregado = addToCart(producto, count, data);
        setCount(1);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      >
              ...
              if (size.id == getValues("selectedSizeId")) {
                return (
                  <div
                    ...
                    className="d-flex justify-content-center align-items-center p-1 border border-secondary rounded shadow-lg"
                  >
                    <button
                      type="button"
                ...
                ?.colors.map((color) => {
                  if (color.id == getValues("selectedColorId")) {
                    return (
                      <div
                        key={color.id}
                        className="d-flex p-1 border border-secondary rounded shadow-lg"
                        title={color.name}
                      >
                        <button
                          type="button"
                    ...
                    return (
                      <div
                        key={color.id}
                        className="d-flex p-1 border border-light rounded "
                      >
                        <button
                          type="button"
                          className="btn rounded "
        ...
        {getValues("selectedSizeId") && getValues("selectedColorId") ? (
          availableStock == 0 ? (
            <div className="contadorControl d-flex align-items-center justify-content-center bg-secondary">
              <button type="button" className="text-white fw-bold" disabled>
                -
              </button>
        ...
        {availableStock == 0 && getValues("selectedColorId") ? (
          <span className="text-danger">Añadiste todo al carrito</span>
        ) : (
          <span className="text-danger invisible">stock agotado</span>
        )}
        ...
        {getValues("selectedSizeId") && getValues("selectedColorId") ? (
          <div className="d-flex flex-column btnAddCart">
            ...
            {availableStock == 0 && (
              <button
                type="button"
                className="btn btn-secondary w-100 rounded-0 text-white"
                disabled
              >
                Añadir al carrito
              </button>
